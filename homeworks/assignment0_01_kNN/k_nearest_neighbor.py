...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        pairwise_product = X @ self.X_train.T
        dists = np.sqrt(X_square + X_train_square - 2* X @ self.X_train.T)
        #dists = 
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            #a_list = list(closest_y)
            #print(a_list)
            #print(type(a_list))
            #d = {x: a_list.count(x) for x in a_list}
            #y_pred[i] = [key for key in d if d[key] == np.max(list(d.values()))][0]
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
